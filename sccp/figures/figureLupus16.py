...
def plotGenePerStatus(X, gene, ax, cellType="Cell Type", weightedCmp=None):
    ...
    df = pd.melt(dataDF, id_vars=["Status", "Cell Type", "Condition"], value_vars=gene).rename(
...
def plot2GenePerCellTypeStatus(X, gene1, gene2, celltype1, celltype2, ax, cellType="Cell Type"):
    """Plots average gene expression across cell types for a category of drugs"""
    adata = X.to_memory()
    gene = [gene1, gene2]
    ...
    for i in range(2):
        ...
        df = pd.melt(dataDF, id_vars=["Status", "Cell Type", "Condition"], value_vars=gene[i]).rename(
            columns={"variable": "Gene", "value": "Value"})
    ...
    df = df.pivot(index=["Status", "Cell Type", "Condition"], columns="Gene", values="Average Gene Expression").reset_index()
