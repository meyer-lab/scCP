...
def distAllGeneDF(data, Pf2s, PCs):
    ...
    for marker in markers:
        ...
        dist_DF = pd.concat([dist_DF, pd.DataFrame({"Marker": [marker], "Normalized Centroid Distance": pf2Dist, "Method": "PARAFAC2"})])
        dist_DF = pd.concat([dist_DF, pd.DataFrame({"Marker": [marker], "Normalzied Centroid Distance": pcaDist, "Method": "PCA"})])
