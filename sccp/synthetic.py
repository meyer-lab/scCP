...
def plotFactors_synthetic(fac, blobXA, n_cluster, ax):
    ...
    yticks = [
        [f"Clst: {i}" for i in np.arange(1, n_cluster + 1)],
        ["X", "Y"],
...
def plotCovFactors_synthetic(fac, blobXA, n_cluster, ax):
    ...
    for i in range(len(covfactors_place)):
        sns.heatmap(
            data=cov_fac[DimCol[covfactors_place[i]]],
            xticklabels=xticks,
            yticklabels=yticks[covfactors_place[i]],
            ax=ax[i + 7],
            cmap=cmap,
            vmin=0,
            vmax=1,
    ...
    for i in range(fac.rank):
        cov_signal = pd.DataFrame(
            covSig[:, :, i] @ covSig[:, :, i].T, columns=yticks[1], index=yticks[1]
        )
