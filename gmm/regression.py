...
def CoH_LogReg_plot(ax, tFac, CoH_Array, numComps):
    ...
    status_DF = pd.read_csv(join(path_here, "gmm/data/CoH_Patient_Status.csv"), index_col=0)
...
def BC_status_plot(cohXA, rank, n_cluster, seed, ax):
    ...
    for i in range(2, rank + 1):
        ...
        accDF = pd.concat([accDF, pd.DataFrame({"Components": [i], "Accuracy (10-fold CV)": np.mean(scoresTFAC)})])
