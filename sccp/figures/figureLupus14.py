...
def makeFigure():
    ...
    for i, status in enumerate(np.unique(df_mean["Status"])):
        for j, celltype in enumerate(np.unique(df_mean["Cell Type"])):
            ...
            ax[1].errorbar(df_mini_mean["APOBEC3A"], df_mini_mean["IFITM3"], xerr=df_mini_std["APOBEC3A"], yerr=df_mini_std["IFITM3"], ls='none', fmt=fmt[i], color=colors[j])
    ...
    ax[1].errorbar(df_mean["APOBEC3A"], df_mean["MGST1"], xerr=df_std["APOBEC3A"], yerr=df_std["MGST1"], ls='none')
    return f
