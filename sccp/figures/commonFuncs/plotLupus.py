...
def plot_all_bulk_pred(X, ax, accuracy_metric="accuracy"):
    ...
    cell_comp_df = cell_comp_df.pivot(
        index=["Condition", "SLE_status", "Processing_Cohort", "condition_unique_idxs"],
        columns="Cell Type",
    ...
    cell_comp_gene_df = aggregate_anndata(
        X, celltype_col="Cell Type", condition_col="Condition", method="Average"
    )
    cell_comp_gene_df = cell_comp_gene_df.sort_values("condition_unique_idxs")
    cell_comp_gene_df.columns = [
        "_".join(col).strip() if isinstance(col, tuple) else col
        for col in cell_comp_gene_df.columns
    ]

    cell_comp_gene_pred = predaccuracy_lupus(
        cell_comp_gene_df, error_metric=accuracy_metric
    )
    y_cell_comp_gene = [cell_comp_gene_pred.iloc[0], cell_comp_gene_pred.iloc[0]]

    combined_df = pd.concat(
        [cell_comp_df.reset_index(), cell_comp_gene_df.reset_index(drop=True)], axis=1
    )
    combined_df = combined_df.set_index(
        ["Condition", "SLE_status", "Processing_Cohort", "condition_unique_idxs"]
    )
    ...
    for ct in cell_types:
        for cond in conditions:
            ...
            if method == "Average":
                ...
                agg_values = (np.sum(group_data.X, axis=0)) / (
                    np.shape(group_data.X)[0]
                )
