...
def makeFigure():
    ...
    sns.scatterplot(
        x=x_comp, y=y_comp, hue="Method", data=scores, palette=palette, s=50, ax=axes[1]
    )
    ...
    for i in range(len(scores_sub) - 1):
        axes[1].plot(
            [scores_sub.iloc[i][x_comp], scores_sub.iloc[i + 1][x_comp]],
            [scores_sub.iloc[i][y_comp], scores_sub.iloc[i + 1][y_comp]],
            color=color,
        )
