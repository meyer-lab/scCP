...
def makeFigure():
    ...
    X = cytotoxic_score(X)
    plotScore(X, ax[0], cellType="Cell Type Old2")
    ax[0].set(title="Cytotoxic Score")
    
    X = interferon_score(X)
    plotScore(X, ax[1], cellType="Cell Type Old2")
    ax[1].set(title="IFN Score")
    
    X = myeloid_score(X)
    plotScore(X, ax[2], cellType="Cell Type Old2")
    ax[2].set(title="Myeloid Score")
...
def myeloid_score(X):  
    ...
    myeloid_genes = df["Gene"].to_numpy()
...
def interferon_score(X):   
    """Scanpy average gene score for all cells"""
    # Obtained from the figure co-expression figure
    interferon_genes = ["ISG15", "RGS1", "EPSTI1", "CD8A", "IFI16", "STAT1", 
                        "HBB", "SP110", "SP100", "EIF2AK2", "IFI27", "IFI44", 
                        "IFI44L", "PATL2", "RNF213", "IFI6", "ISG20", "LAG3", 
                        "MX1", "XAF1", "BST2", "OAS1"]
